...
public:
    ...
    const rcsc::AbstractPlayerObject * teammate( const int unum ) const
      {
          if ( unum < 1 || 11 < unum  )
          {
              std::cerr << "internal error: "
    ...
    const rcsc::AbstractPlayerObject * opponent( const int unum ) const
      {
    ...
    const rcsc::AbstractPlayerCont & allOpponents() const
      {
    ...
    const rcsc::PlayerType * teammatePlayerType( const int unum ) const
      {
          return M_world->teammatePlayerType( unum );
      }

    const rcsc::AbstractPlayerObject * getTeammateGoalie() const
      {
          if ( M_world->teammateGoalieUnum() == rcsc::Unum_Unknown )
          {
              return static_cast< rcsc::AbstractPlayerObject * >( 0 );
          }
    ...
    const rcsc::AbstractPlayerObject * getOpponentGoalie() const
      {
