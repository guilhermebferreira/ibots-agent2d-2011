...
IntentionPassKickFindReceiver::finished( const PlayerAgent * agent )
{
    ...
    const AbstractPlayerObject * receiver = wm.teammate( M_receiver_unum );
...
Bhv_PassKickFindReceiver::execute( PlayerAgent * agent )
{
    ...
    const AbstractPlayerObject * receiver = wm.teammate( pass.targetPlayerUnum() );
                                     ...
                                     const CooperativeAction & pass )
{
    ...
    {
        const AbstractPlayerObject * receiver = agent->world().teammate( receiver_unum );
        if ( ! receiver )
        {
            return;
        }
