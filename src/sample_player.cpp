      ...
      M_action_generator( createActionGenerator() )
{
    ...
    addSayMessageParser( SayMessageParser::Ptr( new JogadaMessageParser( audio_memory ) ) );
...
SamplePlayer::actionImpl()
{
  ...
  static int jogada_coletiva = -1, jogada_coletiva_sender = -1, jogada_coletiva2 = -1;
  static Vector2D destino_sender;
  
  if(((jogada_coletiva != -1)||(jogada_coletiva_sender !=-1)||(jogada_coletiva2!=-1)) &&
    (world().gameMode().type() == GameMode::PlayOn  )  )
	{
	   if((jogada_coletiva+50 >= this->world().time().cycle()) ||
	     (jogada_coletiva2+50 >= this->world().time().cycle() ||
	      (jogada_coletiva_sender+50 >=this->world().time().cycle()) 
	    )
	  )
	   {
	     jogada_coletiva_sender = jogada_coletiva = jogada_coletiva2 = -1;
	   }
	  
	   int qual_jogada = world().audioMemory().jogada().front().qual_jogada_, retorno;
	    ...
	    */
	  //  {	      
	   retorno = Bhv_SetPlay().continuar_jogada
	   (this, qual_jogada, jogada_coletiva, jogada_coletiva2, jogada_coletiva_sender);
	  
	
	   
	   //  std::cout << "\njogador 1 = "<< world().audioMemory().jogada().front().j1_
	     //        << "\njogador 2 = " << world().audioMemory().jogada().front().j2_
	       //      << " \nsender = " << world().audioMemory().jogada().front().sender_ << "\n";
	   switch(retorno)
	   {
	     case 0:
	       return;
    ...
    Bhv_SetPlay().execute( this );
     ...
     //efetua tabelas ....
    if(( ((GameMode::FreeKick_ == world().gameMode().type()) || 
      (GameMode::FreeKickFault_== world().gameMode().type())) || 
      (GameMode::OffSide_== world().gameMode().type())) &&  // se os modos de jogo são os de jogada coletiva
      ( world().gameMode().side() == world().ourSide() ) && // se a bola é para o nosso time
      (world().audioMemory().jogadaTime().cycle() > 0  )) // se existe alguma mensagem na memória de áudio de jogadas
    { //std:: cout << "\n\n---------------------\ncontinuar JOGADA COLETIVA\n---------------------\n\n";
     
      if(( ( world().audioMemory().jogada().front().j1_ == world().self().unum() )|| //se sou o 1º destinatário da mensagem
      ( world().audioMemory().jogada().front().sender_ == world().self().unum() ) // se sou o jogador que enviou a mensagem
      ) &&
      ((world().audioMemory().jogada().front().qual_jogada_ == 0)|| //tabela pela esquerda
      (world().audioMemory().jogada().front().qual_jogada_== 1))  // tabela pela direita
       )
       {
    
    
		    if(world().self().unum() == world().audioMemory().jogada().front().j1_)
		      jogada_coletiva = world().time().cycle(); // recebe o ciclo corrente em que o jogador ouviu a mensagem
		    else if( world().self().unum() == world().audioMemory().jogada().front().sender_ )
		      jogada_coletiva_sender = world().time().cycle(); // recebe o ciclo em que a mensagem foi ouvida...
		    
		    destino_sender = world().audioMemory().jogada().front().jog1_pos_;
		    destino_sender.x = destino_sender.x + 7.5;
		    destino_sender.y = destino_sender.y - 7.5; 
 
      }
      else  if(( ( world().audioMemory().jogada().front().j1_ == world().self().unum() )|| //se sou o 1º destinatário da mensagem
      ( world().audioMemory().jogada().front().j2_ == world().self().unum() ) // se sou o jogador que enviou a mensagem
      )&&
      ((world().audioMemory().jogada().front().qual_jogada_ == 5)|| // meio aberto pela esquerda
      (world().audioMemory().jogada().front().qual_jogada_== 6)|| // meio aberto pela direita
      ( world().audioMemory().jogada().front().qual_jogada_== 2 ) || // lateral com abertura pela esquerda
      (world().audioMemory().jogada().front().qual_jogada_ == 9) || // lateral com abertura pela direita
      (world().audioMemory().jogada().front().qual_jogada_ == 4) || // inversão pela esquerda
      (world().audioMemory().jogada().front().qual_jogada_ == 7)  // inversão pela direita      
      ) )
    {
      if(world().self().unum() == world().audioMemory().jogada().front().j1_)
         jogada_coletiva = world().time().cycle(); // recebe o ciclo corrente em que o jogador ouviu a mensagem
      else if( world().self().unum() == world().audioMemory().jogada().front().j2_ )
        jogada_coletiva2 = world().time().cycle(); // recebe o ciclo em que a mensagem foi ouvida...
      
    }else if((world().audioMemory().jogada().front().qual_jogada_ == 3) || // assistência chute pela esquerda 
      (world().audioMemory().jogada().front().qual_jogada_ == 8)  // assistência chute pela direita
      )
     {
        if(world().self().unum() == world().audioMemory().jogada().front().j1_)
         jogada_coletiva = world().time().cycle(); // recebe o ciclo corrente em que o jogador ouviu a mensagem       
