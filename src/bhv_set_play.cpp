...
#include <../../librcsc-4.0.0/rcsc/action/body_go_to_point2009.h>
...
/* se você for o agente goleiro, faça esse desvio */
    if ( wm.self().goalie() )
    {
...
/* seja você um agente jogador, diferente do goleiro, entre no comando de seleção múltipla */
    switch ( wm.gameMode().type() ) {
    case GameMode::KickOff_: /* começo de jogo, bola no meio campo */
        ...
        break;
    case GameMode::KickIn_: /* lateral e escanteio, possuem o mesmo tratamento */
    ...
    case GameMode::IndFreeKick_:
        ...
        break;
    case GameMode::FoulCharge_: // cobrar falta de 2 toques
    ...
    case GameMode::FreeKickFault_: /* falta */
    case GameMode::FreeKick_:  /* tiro livre */ 
    case GameMode::OffSide_: /* impedimento */
      if ( wm.gameMode().side() == wm.ourSide() ) //se a bola for de nosso time...
        {
           return Bhv_SetPlayFreeKick().jogada_coletiva_call(agent);
	}
      break;
    ...
    case GameMode::CornerKick_: /* escanteio, já foi tratado com kickin que é o lateral  */
...
Bhv_SetPlay::is_kicker( const PlayerAgent * agent )
{
    ...
    if ( ! kicker )
    {
        ...
        return true;
                ...
                // eu sou o jogador mais próximo da bola ? ou sou o segundo mais próximo ?
    if ( kicker
         && second_kicker
...
switch (qual_jogada)
{
  ...
  case 1: //tabela pela esquerda
    continuar_tabela_esq
    (agent, jogada_coletiva, jogada_coletiva_sender );
    break;
  case 2: //lateral com lançamento pela esquerda
    return continuar_lateral_lanc_esq 
    (agent,   jogada_coletiva,  jogada_coletiva2 );
    break;
  case 3: //assistência chute pela esquerda
    return continuar_assistencia_esq
    (agent,  jogada_coletiva,  jogada_coletiva2 );
    break;
  case 4: //inversão pela esquerda
    return continuar_inversao_esq
    (agent,   jogada_coletiva,  jogada_coletiva2 );
    break;
  case 5://lancamento pelo meio, com abertura pela lateral esquerda
    return continuar_meio_aberto_lateral_esq
    (agent,   jogada_coletiva,  jogada_coletiva2 );
    break;
  case 6://lançamento pelo meio com abertura pela lateral direita
    return continuar_meio_aberto_lateral_dir
    (agent, jogada_coletiva,  jogada_coletiva2 );
    break;
  case 7://inversão direita
    return continuar_inversao_dir
    (agent,  jogada_coletiva,  jogada_coletiva2);
    break;
  case 8://assistência para chute pela direita
    return continuar_assistencia_dir
    (agent,  jogada_coletiva,  jogada_coletiva2 );
    break;
  case 9://correr e cruzar pela direita
    return continuar_lateral_lanc_dir
    (agent,  jogada_coletiva,  jogada_coletiva2 );
    break;
  case 0: //tabela pela direita
    return continuar_tabela_dir
    (agent,  jogada_coletiva,  jogada_coletiva_sender );
    break;
...
int
Bhv_SetPlay::
continuar_tabela_dir
( rcsc::PlayerAgent * agent,  int jogada_coletiva,  int jogada_coletiva_sender )
{
	    if((jogada_coletiva != -1) && //se você ouviu a mensagem nos últimos 15 ciclos, dê continuidade à jogada...
	     ((jogada_coletiva+15) >=agent->world().time().cycle()) )
	  {
	    
		if(agent->world().self().unum() == agent->world().audioMemory().jogada().front().j1_)
		{
		  if( agent->world().self().isKickable() )//tente fazer isso durante 3 ou 4 ciclos...
		  { // std::cout << "\nxxxxxxxxxx\ndá pra chutar \nxxxxxxxxxx\n";
		    int num_jog = agent->world().audioMemory().jogada().front().sender_;
		   
			    Vector2D target_point = agent->world().teammate(num_jog)->pos();// destino_sender; //world().self().pos();
			    target_point.x += 4.5;
			    target_point.y += 1.0;
			    
		    if (  (   agent->world().teammate(num_jog)->pos().x > -30)
			  ||(  agent->world().teammate(num_jog)->pos().dist
			  (target_point)< 10.0) )
			{
			   // std::cout << "\nxxxxxxxxxx\nposição legal\nxxxxxxxxxx\n";
			  
			  const double max_ball_speed = agent->world().self().kickRate() * ServerParam::i().maxPower();
			    double ball_move_dist = agent->world().ball().pos().dist( target_point );
			    int ball_reach_step
			    ...
			    Body_KickOneStep:: Body_KickOneStep( target_point, ball_speed ).execute( agent );
			    agent->setNeckAction( new Neck_ScanField() );   
			    
		    /*
		     * std:: cout << "\n\n---------------------\nDava pra Devolver a bola\nsou jogador de numero = " 
			  << world().self().unum()
			  << "\nnumero do sender eh = "<< world().audioMemory().jogada().front().sender_
			  <<"\nexistem " << world().audioMemory().jogada().size()
			  <<" mensagens de áudio, no vetor de memória\n---------------------\n\n";
		     */
		  }
		  	  
			  jogada_coletiva = -1;
			  return 1; //o parâmetro "jogada_coletiva" é setado para -1 
		  }
		  else
		  {
		   // std::cout << "\nYYYYYYYYYY\nNÃO dá pra chutar \nYYYYYYYYYY\n";
		 ...
		 // agent->doPointto();
		  return 0; //retorna na função superior
		    
		  }
		}
	 
	  }else if((jogada_coletiva_sender!=-1)&&
		  ((jogada_coletiva_sender+15)>= agent->world().time().cycle())
	  )
	  {
	    if( agent->world().self().unum() == agent->world().audioMemory().jogada().front().sender_ )
		{ //std::cout << "\n++++++++++\n sender indo para o ponto \n++++++++++\n";
		 
	       if( ! agent->world().self().isKickable() ) //se não dá pra chutar
	       {
	        Vector2D target_point;// = destino_sender;
			    target_point.x = agent->world().audioMemory().jogada().front().jog1_pos_.x + 3.5;
			    target_point.y = agent->world().audioMemory().jogada().front().jog1_pos_.y-2.9;
		  double dash_power
	      = Bhv_SetPlay::get_set_play_dash_power( agent ); // captura a potencia de velocidade do agente
		
	       double dist_thr;
	      	
		dist_thr = agent->world().self().pos().dist(target_point) * 0.07;
		if ( dist_thr < 0.5 ) 
		{
		 dist_thr = 0.5; 
		 jogada_coletiva_sender = -1;
		}
		...
		agent->setNeckAction( new Neck_ScanField() );
		//Bhv_BodyNeckToBall::Bhv_BodyNeckToBall().execute(this) ;
		  return (jogada_coletiva_sender==-1)?3:0; //parâmetro "jogada_coletiva_sender" deve receber -1 na função superior 
			  //std::cout << "\n\n---------------------\nPLAY ON APÓS JOGADA\nsou jogador que enviou a mensagem,\nmeu numero eh = " 
			  //<< world().self().unum()
			  //<<"\n---------------------\n\n";
			 
	       }
		//Bhv_SetPlay::jogada_coletiva = -1;
		}
	    
	  }else // não precisa tentar continuar a jogada mais, é inviável...
	  {
	   // std::cout << "\n-------------------\nnão vou mais tentar executar jogada coletiva \n------------------- \n";
	    jogada_coletiva = jogada_coletiva_sender = -1;
	    agent->setNeckAction( new Neck_ScanField() );
	    return 2; // os dois parâmetros são setados para -1;
	    
	  }
  agent->setNeckAction( new Neck_ScanField() );
  return 4; //continue execução
}

//-------------------------------------------------------------------------------------------------

int Bhv_SetPlay::continuar_tabela_esq
( rcsc::PlayerAgent * agent, int jogada_coletiva, int jogada_coletiva_sender )
{
     if((jogada_coletiva != -1) && //se você ouviu a mensagem nos últimos 10 ciclos, dê continuidade à jogada...
	     ((jogada_coletiva+15) >=agent->world().time().cycle()) )
	  {
		...
		if(agent->world().self().unum() == agent->world().audioMemory().jogada().front().j1_)
		{
		  ...
		  { // std::cout << "\nxxxxxxxxxx\ndá pra chutar \nxxxxxxxxxx\n";
		    int num_jog = agent->world().audioMemory().jogada().front().sender_;
			    ...
			    Vector2D target_point = agent->world().self().pos();// destino_sender; //world().self().pos();
			    target_point.x += -0.5; //-0.9;
			    target_point.y += 1.6;
		  ...
		  else
		  {
		   // std::cout << "\nYYYYYYYYYY\nNÃO dá pra chutar \nYYYYYYYYYY\n";
		  agent->setNeckAction( new Neck_ScanField() );
		  agent->doPointto(agent->world().ball().pos().x,agent->world().ball().pos().y);		 
		//  agent->doTurn(); // doMove(agent->world().ball().pos().x,agent->world().ball().pos().y);		  
		   Body_GoToPoint2009::Body_GoToPoint2009
		   (agent->world().ball().pos(),
		    agent->world().self().distFromBall()+2,
		    Bhv_SetPlay::get_set_play_dash_power( agent )).execute( agent ) ;
		  return 0; //retorna na função superior
	  ...
	  )
	  {
		...
		{
		   agent->doMove(agent->world().ball().pos().x,agent->world().ball().pos().y);
		  target_point.x = agent->world().ball().pos().x;
		  target_point.y = agent->world().ball().pos().y+3.0;
		}
		else
		{        
		  target_point.x = agent->world().self().pos().x + 10; //agent->world().audioMemory().jogada().front().jog1_pos_.x + 3.5;
		  target_point.y = agent->world().self().pos().y + 2; //agent->world().audioMemory().jogada().front().jog1_pos_.y-2.9;
		}
		  double dash_power
	      = Bhv_SetPlay::get_set_play_dash_power( agent ); // captura a potencia de velocidade do agente
...
int
Bhv_SetPlay::continuar_meio_aberto_lateral_dir
( rcsc::PlayerAgent * agent, int jogada_coletiva, int jogada_coletiva2 )
{
    if((jogada_coletiva != -1) && //se você ouviu a mensagem nos últimos 8 ciclos, dê continuidade à jogada...
	     ((jogada_coletiva+10) >=agent->world().time().cycle()) )
	  {
		...
		if(agent->world().self().unum() == agent->world().audioMemory().jogada().front().j1_)
		{
		  if( agent->world().self().isKickable() )//tente fazer isso durante 3 ou 4 ciclos...
		  { // std::cout << "\nxxxxxxxxxx\ndá pra chutar \nxxxxxxxxxx\n";
		    int num_jog = agent->world().audioMemory().jogada().front().j2_;
	...
	//	  }
	  ...
	  }else if((jogada_coletiva2!=-1)&&
		  ((jogada_coletiva2+8)>= agent->world().time().cycle())
	  )
	  {
	    if( agent->world().self().unum() == agent->world().audioMemory().jogada().front().j2_ )
		{ //std::cout << "\n++++++++++\n sender indo para o ponto \n++++++++++\n";
		 
	          agent->setNeckAction( new Neck_ScanField() );
		  agent->doPointto
		  (agent->world().teammate
		  (agent->world().audioMemory().jogada().front().j1_)->pos().x,
		  agent->world().teammate
		  (agent->world().audioMemory().jogada().front().j1_)->pos().y);
	        return 4;
	  ...
	  {
	   // std::cout << "\n-------------------\nnão vou mais tentar executar jogada coletiva \n------------------- \n";
	    jogada_coletiva = jogada_coletiva2 = -1;
	    return 5; // os dois parâmetros (jogada_coletiva e jogada_coletiva2 ) são setados para -1;
...
//--------------------------------------------------------------------------------------------------------------
int Bhv_SetPlay::continuar_meio_aberto_lateral_esq 
( rcsc::PlayerAgent * agent, int jogada_coletiva, int jogada_coletiva2 )
{
      if((jogada_coletiva != -1) && //se você ouviu a mensagem nos últimos 3 ciclos, dê continuidade à jogada...
	     ((jogada_coletiva+8) >=agent->world().time().cycle()) )
	  {
...
int Bhv_SetPlay::continuar_lateral_lanc_dir 
( rcsc::PlayerAgent * agent, int jogada_coletiva, int jogada_coletiva2 )
{
  if((jogada_coletiva != -1) && //se você ouviu a mensagem nos últimos 3 ciclos, dê continuidade à jogada...
	     ((jogada_coletiva+10) >=agent->world().time().cycle()) )
	  {
		...
		if(agent->world().self().unum() == agent->world().audioMemory().jogada().front().j1_)
		{
		 ...
		 // agent->doPointto(agent->world().ball().pos().x,agent->world().ball().pos().y);
		  ...
		  ((jogada_coletiva2+10)>= agent->world().time().cycle())
	  )
	  {
	    if( agent->world().self().unum() == agent->world().audioMemory().jogada().front().j2_ )
		{ //std::cout << "\n++++++++++\n sender indo para o ponto \n++++++++++\n";
...
int Bhv_SetPlay::continuar_lateral_lanc_esq 
( rcsc::PlayerAgent * agent, int jogada_coletiva, int jogada_coletiva2 )
{
    if((jogada_coletiva != -1) && //se você ouviu a mensagem nos últimos 3 ciclos, dê continuidade à jogada...
	     ((jogada_coletiva+10) >=agent->world().time().cycle()) )
...
int Bhv_SetPlay::continuar_inversao_dir 
( rcsc::PlayerAgent * agent, int jogada_coletiva, int jogada_coletiva2 )
{
   if((jogada_coletiva != -1) && //se você ouviu a mensagem nos últimos 10 ciclos, dê continuidade à jogada...
	     ((jogada_coletiva+10) >=agent->world().time().cycle()) )
...
int Bhv_SetPlay::continuar_inversao_esq 
( rcsc::PlayerAgent * agent,  int jogada_coletiva, int jogada_coletiva2 )
{
  if((jogada_coletiva != -1) && //se você ouviu a mensagem nos últimos 3 ciclos, dê continuidade à jogada...
	     ((jogada_coletiva+10) >=agent->world().time().cycle()) )
...
int Bhv_SetPlay::continuar_assistencia_dir
( rcsc::PlayerAgent * agent, int jogada_coletiva, int jogada_coletiva2 )
{
	     ...
	     ((jogada_coletiva+10) >=agent->world().time().cycle()) )
	  {
		...
		if(agent->world().self().unum() == agent->world().audioMemory().jogada().front().j1_)
		{
		  ...
		  /*  chutar para o gol, mas como?   */
		  Body_ForceShoot::Body_ForceShoot().execute(agent);
  ...
  return (jogada_coletiva2!=-1)?6:4; //continue execução
}

//-----------------------------------------------------------------------------------------------

int Bhv_SetPlay::continuar_assistencia_esq
( rcsc::PlayerAgent * agent, int jogada_coletiva, int jogada_coletiva2)
{   // std::cout << "\n In esquerda \n";
  	    if((jogada_coletiva != -1) && //se você ouviu a mensagem nos últimos 10 ciclos, dê continuidade à jogada...
	     ((jogada_coletiva+10) >=agent->world().time().cycle()) )
	  {
	   // std::cout << "\n assistência pela esquerda \n";
		if(agent->world().self().unum() == agent->world().audioMemory().jogada().front().j1_)
		{
		  if( agent->world().self().isKickable() )//tente fazer isso durante 3 ou 4 ciclos...
		  { // std::cout << "\nxxxxxxxxxx\ndá pra chutar \nxxxxxxxxxx\n";
