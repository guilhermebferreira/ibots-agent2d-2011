...
if (wm.gameMode().type() == GameMode::FreeKick_ )
 {
   //std::cout << "\ntiro livre\n";
  // sayJogada(agent);
}else if(wm.gameMode().type() == GameMode::CornerKick_)
{
    //std::cout << "\nescanteio\n";   
   // sayJogada(agent);
}else if(wm.gameMode().type() == GameMode::OffSide_)
{
  //std::cout << "\nimpedimento\n"; 
  //sayJogada(agent);
}
 ...
 */
bool
SampleCommunication::sayJogada( PlayerAgent * agent )
{
    const int current_len = agent->effector().getSayMessageLength();
    if ( current_len == 0 )
    {
    ...
    /*
     * Nesse trecho de código deve ser chamada a função Fuzzy que vai passar os parâmetros indicados  
     * na mensagem de áudio.
     *  
     */
 Vector2D jog;
 jog.x=0;
 jog.y=0;
 int numerojogador1 = 2;
 int numerojogador2 = 3;
 jogadas tipo_jogada = tabela_dir;
 
    agent->addSayMessage( new JogadaMessage(tipo_jogada,jog, jog,numerojogador1, numerojogador2 ) ); 
    //supondo que x indique que 2 jogadores pré-determinados participarão da jogada, isso pode significar, por exemplo,
    //que quando o argumento valer "x" os jogadores que participarão serão os jogadores 3 e 4 

    dlog.addText( Logger::COMMUNICATION,
                  __FILE__": say jogada message" );
    return true;
}
